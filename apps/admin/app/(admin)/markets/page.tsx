...
interface PageProps {
  searchParams: Promise<{ page?: string; status?: string; category?: string; source?: string }>;
}

export default async function MarketsPage({ searchParams }: PageProps) {
  const { page, status, category, source } = await searchParams;
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-5">
        {["", "ai"].map((src) => (
          <Link key={src} href={src ? `/markets?source=${src}${status ? `&status=${status}` : ""}` : `/markets${status ? `?status=${status}` : ""}`}
            className={`text-xs font-semibold uppercase tracking-wider px-3 py-1.5 rounded-full border transition-colors ${
              (source ?? "") === src ? "border-green text-green" : "border-border text-text-secondary hover:text-text-primary"
            }`}>
            {src || "All Sources"}
          </Link>
        ))}
            ...
            {data.markets.map((m: any) => (
              <tr key={m.id} className="border-b border-border last:border-0 hover:bg-surface-3 transition-colors">
                <td className="p-4 text-text-primary font-medium">
                  {m.title}
                  {m.source === "ai" && (
                    <span className="ml-2 text-xs font-bold px-1.5 py-0.5 rounded bg-green/15 text-green">AI</span>
                  )}
                </td>
